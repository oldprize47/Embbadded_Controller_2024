...
{ // trigmode 0 : SW, 1 : TRGO
	...
	GPIO_pupd(pinName, NOPUPD); // EC_NONE = 0
...
void ADC_trigger(TIM_TypeDef *TIMx, int msec, int edge)
{
	...
	else if (edge == ADC_FALL)
		ADC1->CR2 |= ADC_CR2_EXTEN_1; // trigger detection falling edge
	else if (edge == ADC_BOTH)
		ADC1->CR2 |= ADC_CR2_EXTEN_Msk; // trigger detection both edge
}
...
void JADC_trigger(TIM_TypeDef *TIMx, int msec, int edge)
{
	...
	if (TIMx == TIM1)
		ADC1->CR2 |= timer << 16; // TIMx TRGO event (JADC : TIM1, TIM2, TIM4, TIM5 TRGO)
	else if (TIMx == TIM2)
		ADC1->CR2 |= (timer + 1) << 16; // TIMx TRGO event (JADC : TIM1, TIM2, TIM4, TIM5 TRGO)
	else
		ADC1->CR2 |= (timer * 2 + 1) << 16; // TIMx TRGO event (JADC : TIM1, TIM2, TIM4, TIM5 TRGO)

	ADC1->CR2 &= ~ADC_CR2_JEXTEN; // reset JEXTEN, default
	if (edge == ADC_RISE)
		ADC1->CR2 |= ADC_CR2_JEXTEN_0; // trigger detection rising edge
	else if (edge == ADC_FALL)
		ADC1->CR2 |= ADC_CR2_JEXTEN_1; // trigger detection falling edge
	else if (edge == ADC_BOTH)
		ADC1->CR2 |= ADC_CR2_JEXTEN; // trigger detection both edge
}
